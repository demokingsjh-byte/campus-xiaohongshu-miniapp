...
1. 复用 RuoYi-Vue-Pro 的 `system_tenant` 做 SaaS 租户底座。
2. 校区不是单独再造租户中间件，而是给 `system_tenant` 增加校园业务资料。
3. 所有校园业务表都带 `tenant_id`，直接复用 RuoYi 的租户拦截、数据隔离、权限体系。
...
这个 SQL 应在 RuoYi 原始 `sql/mysql/ruoyi-vue-pro.sql` 之后执行。

## 后台菜单建议

在 RuoYi 后台新增一级菜单：`校园运营`
...
3. 使用 RuoYi 代码生成器生成 `campus_*` 表的 CRUD。
